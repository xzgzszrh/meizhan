...
export async function onRequestPost(ctx) {
  try {
    ...
    const uriParams = {
      state: Array.from(crypto.getRandomValues(new Uint32Array(ctx.data.userid.length)), (dec) =>
    ...
    const r = await ctx.data.Http.post(`/api/connectors/${connectorID.id}/authorization-uri`, {
      data: uriParams
    })
    return json(r)
  } catch (e) {
